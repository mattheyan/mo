...
function Invoke-Application {
	...
	if ($ReturnType -eq "Output") {
	    ...
	    if ($err.Length -ne 0 -and (-not($ValidExitCodes -contains $exitCode) -or !$SuppressInconsistentErrorLogging)) {
	        Write-Host $err.ToString() -ForegroundColor Red
	    }
	
	    if ($EnsureSuccess -eq $true) {
	        if(-not($ValidExitCodes -contains $exitCode)) {
	...
	elseif ($RunAsAdmin.IsPresent -and (-NOT ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator"))) {
	    ...
	    if ($EnsureSuccess -eq $true) {
	        if(-not($ValidExitCodes -contains $process.ExitCode)) {
...
function Invoke-ApplicationWithOptions {
	...
	Invoke-Application -FilePath $appInfo.Definition -Arguments $arguments -EnsureSuccess $EnsureSuccess -ValidExitCodes $ValidExitCodes -ReturnType $ReturnType -Logging $Logging -RunAsAdmin:$RunAsAdmin
}
